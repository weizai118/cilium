...
func (kub *Kubectl) GeneratePodLogGatheringCommands(reportCmds map[string]string) {
	...
	for _, pod := range pods {
		for _, containerStatus := range pod.Status.ContainerStatuses {
			...
			logfileName := fmt.Sprintf("%s-%s-%s.log", pod.Namespace, pod.Name, containerStatus.Name)
			...
			if containerStatus.RestartCount > 0 {
				...
				previousLogfileName := fmt.Sprintf("%s-%s-%s-previous.log", pod.Namespace, pod.Name, containerStatus.Name)
